...
function [lowTri,upTri,P] = factorizationLR(A) 
  ...
  for pivot = 1:r
    # FULL PIVOTING -- causes error in larger matrix, haven't found bug yet
    P= P;#findPivot(m,P,r,pivot);
...
function [Q,R] = factorizationQR(A)
  ...
  for j = 1:r
    rjj_sum = 0;
    for i = 1:c
      rjj_sum = rjj_sum + (Q(i,j)^2);
    endfor
    R(j,j) = sqrt(rjj_sum);
    
    if (R(j,j) == 0) 
      disp("A has a linearly dependent column");
      return;
    else 
      Q(:,j) = Q(:,j) / R(j,j);
    endif
...
function x = solveEquation(A,b,method) 
  ...
  elseif (strcmp(method,"QR"))
    disp("Using QR Factorization with transposed matrix multiplication and backward substitution");
    [q,r] = factorizationQR(A);
    x = transpose(backwardSubstitution(r,transpose(q)*b));
  endif
